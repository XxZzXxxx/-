...
public:
  ...
  void divideMapAndFindCenters();
  const std::array<Eigen::Vector3d, 16>& get_map_position() const { return map_position; }
  const std::array<int, 16>& get_occ_map() const { return occ_map; }
  void generateRandomTargets();
  void publishMarkers();
  void executeSearch(int drone_id, const Eigen::Vector3d& current_pos);
  void executeCheck(int drone_id, int target_idx);
  void executeAttack(int drone_id, int target_idx);
  bool executeTask(int drone_id, const std::string& task_type, const Eigen::Vector3d& current_pos);
  int findNearestTarget(const Eigen::Vector3d& pos);
  int findNearestRealTarget(const Eigen::Vector3d& pos);
...
private:
  ...
  std::vector<bool> target_types_;
  std::vector<bool> visited_targets_;
  ...
  Eigen::Vector3d findSuitablePoint(const Eigen::Vector3d& center, const Eigen::Vector3d& cell_size);
