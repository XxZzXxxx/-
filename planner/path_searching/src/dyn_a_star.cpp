...
double AStar::calculatePathCost(const Eigen::Vector3d& start, const Eigen::Vector3d& end)
{
    Eigen::Vector3i start_idx, end_idx;
    if (!Coord2Index(start, start_idx) || !Coord2Index(end, end_idx))
    {
        return std::numeric_limits<double>::max();
    }

    if (AstarSearch(step_size_, start, end))
    {
