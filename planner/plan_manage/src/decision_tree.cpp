...
AStar::Ptr astar_ = std::make_shared<AStar>();
...
TASK_NODE::TASK_NODE(int drone_num, std::shared_ptr<AStar> astar_ptr,int virtual_target, int true_target)
    : drone_num(drone_num), astar(astar_ptr), virtual_target_(virtual_target), true_target_(true_target)
 {
...
Eigen::Vector3d TASK_NODE::getNextSearchPosition(int drone_id) {
        return choosenextwaypoint(current_position, drone_id, astar);
    }
...
void DecisionTree::first_layer_decision(const TASK_NODE::Ptr& task_node, const SimulationResult& current_state){
...
DecisionTree::FirstDecision DecisionTree::backtrack(int depth, double current_cost, const SimulationResult& current_state, const TASK_NODE::Ptr& task_node) {
    ...
    for (int task = 0; task < 3; ++task) {
        ...
        SimulationResult tmp = {new_virtual_target, new_true_target};
        SimulationResult new_state = current_state + tmp;
...
double DecisionTree::simulate_mission_cost(int task, int drone_id, const TASK_NODE::Ptr& task_node, double current_cost) {
    ...
    SimulationResult current_state_ = task_node->get_current_state();
    if (!task_node->precon(drone_id, task, current_state_.virtual_targets, current_state_.true_targets)) {
...
void Decision::decision_thread_function() {
    ...
    try {
            ...
            ROS_ERROR_STREAM("Exception in decision making: " << e.what());
        }
...
void Decision::initDecision(ros::NodeHandle &nh,  std::shared_ptr<GridMap> grid_map) {
    nh_ = nh;
    ...
    astar->initGridMap(grid_map_, Eigen::Vector3i(100, 100, 100));
    double step_size = 0.1; // 或者从参数服务器获取
    astar->setStepSize(step_size);

    // 获取地图信息
    map_position = grid_map_->get_map_position();
    occ_map = grid_map_->get_occ_map();
    
    task_node = std::make_shared<TASK_NODE>(drone_num, std::make_shared<AStar>(),0, 0);
    ...
    for (int i = 0; i < drone_num; i++) {
        ...
        auto planner = std::make_shared<ego_planner::EGOReplanFSM>(grid_map_);
        planner->setGridMap(grid_map);
        planner->init(nh);
        planner->setDroneId(i);
        ego_planners.push_back(planner);
...
void Decision::execute_tasks() {
   for (int i = 0; i < drone_num; i++) {
            ...
            if (decision_tree->has_decided && !decision_tree->has_executed_first_layer) {
                ...
                string Task = task_node->task_name_[task];
                // 获取起点和终点
                Eigen::Vector3d start_pt = ego_planners[i]->getOdomPosition();
                Eigen::Vector3d start_vel = ego_planners[i]->getOdomVelocity();
                Eigen::Vector3d start_acc = ego_planners[i]->getOdomAcceleration();
                ...
                // 使用EGO Planner生成轨迹
                ego_planners[i]->planGlobalTraj(start_pt, start_vel, start_acc, end_pt, end_vel, end_acc);
                ego_planners[i]->setCurrentTask(Task);
...
void Decision::odomCallback(const nav_msgs::OdometryConstPtr &msg, int drone_id) {
    ...
    Eigen::Vector3d current_pos = ego_planners[drone_id]->getOdomPosition();
...
void Decision::continue_current_task(int drone_id, const Eigen::Vector3d& current_pos, const Eigen::Vector3d& target_pos) {
    // 获取当前速度和加速度
    Eigen::Vector3d current_vel = ego_planners[drone_id]->getOdomVelocity();
...
void Decision::update_task_results(int drone_id, const std::string& task_result, const geometry_msgs::Vector3& position_) {
    Eigen::Vector3d position = Eigen::Vector3d(position_.x, position_.y, position_.z);
    task_node->update_drone_status(drone_id, position, task_node->current_drone_tasks[drone_id]);

    if (task_result == "search") {
        task_node->add_virtual_target(position);
    } else if (task_result == "check true") {
        task_node->true_target_++;
        task_node->remove_virtual_target(position);
        task_node->true_target_positions.push_back(position);
    } else if(task_result == "check false"){
      task_node->virtual_target_--;
      task_node->remove_virtual_target(position);
    }else if (task_result == "attack") {
        task_node->true_target_--;
    }
