...
struct SimulationResult {
    int virtual_targets;
    int true_targets;
    SimulationResult operator+(const SimulationResult& other) const {
        return {virtual_targets + other.virtual_targets, true_targets + other.true_targets};
    }
};//存储模拟结果和真实结果
...
class TASK_NODE{
    ...
    public:
        ...
        TASK_NODE(int drone_num, std::shared_ptr<AStar> astar_ptr,int virtual_target, int true_target );
...
class Decision
{
    ...
    public:
        ...
        Decision(ros::NodeHandle &nh,  std::shared_ptr<GridMap> grid_map);
        ~Decision();
        // void decision_making(const ros::TimerEvent &e);
        void decision_making();
        void initDecision(ros::NodeHandle &nh, std::shared_ptr<GridMap> grid_map);
        void update_status(int id, Eigen::Vector3d position);
        void odomCallback(const nav_msgs::OdometryConstPtr &msg, int drone_id);
        void taskResultCallback(const shared_msgs::TaskResultConstPtr& msg);
        void execute_tasks();
        bool check_replanning_condition();
        void replan();
        void update_task_results(int drone_id, const std::string& task_result, const geometry_msgs::Vector3& position_);
